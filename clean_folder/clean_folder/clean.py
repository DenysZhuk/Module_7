...
def arrange_folder(target_path: str, destination_folder: str = None):
    ...
    inner_files = os.listdir(target_path)  # функція повертає список файлів, що є у папці
    for filename in inner_files:  # для файлу в списку ми
        file_path: str = os.path.join(target_path, filename)  # формуємо новий шлях за допомогою джоін
        if os.path.isdir(file_path):  # перевіряємо чи файл є папкою
            arrange_folder(file_path, destination_folder)  # якщо є ми йому назначаємо новий шлях
        elif os.path.isfile(file_path):  # перевіряємо чи файл є файл
            move_to_category_folder(file_path, destination_folder)  # якщо це файл назначаємо новий шлях
